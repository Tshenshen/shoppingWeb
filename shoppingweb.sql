...
CREATE TABLE `tbl_enterprise`  (
  ...
  `balance` double(11, 2) NULL DEFAULT 0.00 COMMENT '商家余额',
...
CREATE TABLE `tbl_favourite`  (
  ...
  `tag_dic_id` varchar(50) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '标签值id',
  `point` int(0) NULL DEFAULT NULL COMMENT '分数',
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE INDEX `index_userid_tagid`(`user_id`, `tag_dic_id`) USING BTREE COMMENT '用户id和标签id的唯一索引'
) ENGINE = InnoDB CHARACTER SET = utf8 COLLATE = utf8_general_ci ROW_FORMAT = Dynamic;
